...
function Game(id,params){
    ...
    var Item = function(params){
        ...
        this._settings = {
            ...
            coord:null,				//如果对象与地图绑定,需设置地图坐标;否则设置位置坐标
            path:[],				//NPC自动行走的路径
            vector:null,			//目标坐标
            //布局相关
    ...
    Map.prototype.finder = function(params){
        ...
        var finded = false;
        var result = [];
        var y_length  = options.map.length;    //map列的长度
        var x_length = options.map[0].length;  //map行的长度
        var steps = [];     //步骤的映射
    ...
    //播放音频
    Stage.prototype.musicplay = function(num) {
        var start = document.getElementById("staMusic");
        var eat = document.getElementById("eatMusic");
        var lose = document.getElementById("loseMusic");
        var win = document.getElementById("winMusic");
        var death = document.getElementById("dieMusic");
        var eatgoods = document.getElementById("goodsMusic");
        var eatghost = document.getElementById("ghostMusic");
        switch (num) {
            case 0:
                eat.pause();eat.currentTime = 0;
                lose.pause();lose.currentTime = 0;
                win.pause();win.currentTime = 0;
                death.pause();death.currentTime = 0;
                eatgoods.pause();eatgoods.currentTime = 0;
                eatghost.pause();eatghost.currentTime = 0;
                break;
            case 1:start.play();break;
            case 2:eat.play();break;
            case 3:lose.play();break;
            case 4:win.play();break;
            case 5:death.play();break;
            case 6:eatgoods.play();break;
    ...
    this.start = function() {
        ...
        var fn = function(){
            ...
            _context.fillRect(0,0,_.width,_.height);        //背景全黑
            f++;
            if(stage.timeout){
                stage.timeout--;
            }
